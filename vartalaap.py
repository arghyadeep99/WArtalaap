...
st.title('Vartalaap: Analyze your WhatsApp Chats')
st.markdown('Analyze your personal chats or group chats with Vartalaap!')

st.sidebar.title("Vartalaap")
...
if uploaded_file is not None:
	...
	if option == 'All':
		...
		st.text("Hover on Chart to see details.")
		st.plotly_chart(daily_freq_plot(messages_df),use_container_width=True)
		st.subheader("**The most happening day for %s was - **"% name)
		most_active_days(messages_df)
		st.pyplot()
		time.sleep(0.2)
		st.subheader("**When is %s most active?**"% name)
		most_active_time(messages_df)
		st.pyplot()
		time.sleep(0.2)
		st.subheader("**%s's WordCloud **"% name)
